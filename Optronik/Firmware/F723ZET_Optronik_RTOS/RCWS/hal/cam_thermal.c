...
#if CAMERA_SWAP==1
static TBT_interface_t tbt = { .port_fd = &huart4 };
#else
static TBT_interface_t tbt = { .port_fd = &huart2 };
...
{
	if (TBT_set_focus_start(&tbt, TBT_FOCUS_START_NEAR, (speed & TBT_FOCUS_START_SPEED_MAX)) == TBT_SUCCESS)
...
void tbt_man_focus_near_start()
{
	...
	TBT_FOCUS_START_SPEED_MAX) == TBT_SUCCESS)
		LOG("man focus near start\r\n");
	else
		LOG_E("failed\r\n");
}
...
{
	if (TBT_set_focus_start(&tbt, TBT_FOCUS_START_FAR, (speed & TBT_FOCUS_START_SPEED_MAX)) == TBT_SUCCESS)
...
{
	if (TBT_set_video_mode(&tbt, mode))
		LOG("video= PAL\r\n");
}
