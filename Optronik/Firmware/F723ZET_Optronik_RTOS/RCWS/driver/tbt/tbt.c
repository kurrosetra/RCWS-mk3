...
{
	if (HAL_UART_Transmit(iface->port_fd, iface->oBuf, TBT_OUTPUT_BUFFER_SIZE, TBT_SERIAL_WAIT) != HAL_OK)
		return TBT_FAILURE;
...
uint32_t TBT_set_zoom_optical_start(TBT_interface_t *iface, const uint8_t zoom_end, const uint8_t speed)
{
	memcpy(iface->oBuf, tbt_default_output_format, TBT_OUTPUT_BUFFER_SIZE);
...
uint32_t TBT_set_zoom_optical_direct(TBT_interface_t *iface, const uint16_t zoom, const uint8_t speed)
{
	memcpy(iface->oBuf, tbt_default_output_format, TBT_OUTPUT_BUFFER_SIZE);
...
uint32_t TBT_set_video_mode(TBT_interface_t *iface, const uint8_t mode)
{
	memcpy(iface->oBuf, tbt_default_output_format, TBT_OUTPUT_BUFFER_SIZE);

	iface->oBuf[1] = TBT_VIDEO_MODE;
	iface->oBuf[2] = mode;
