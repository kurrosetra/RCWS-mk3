...
//	_NOPTEL_write_packet_data(&noptel->iface, 0);
	_NOPTEL_write_packet_data_with_reply(&noptel->iface);
//	if (_NOPTEL_write_packet_data_with_reply(&noptel->iface) == HAL_OK) {
//		if (_parse_incoming_packet(&noptel->iface, &startPos) == HAL_OK) {
//			if (noptel->iface.ibuf[startPos + 1] == NOPTEL_SET_POINTER_MODE) {
//				printf("%dB -> ", noptel->iface.ilen);
//				for ( int var = 0; var < noptel->iface.ilen; var++ ) {
//					printf("[%02X]", noptel->iface.ibuf[var]);
//				}
//				printf("\r\n");
//			}
//
