...
#if MOTOR_DRIVER_DEBUG==1
TSerial *serial;
#endif	//if MOTOR_DRIVER_DEBUG==1
...
FDCAN_TxHeaderTypeDef mtrTxHeader;
//uint32_t canTxMailBox;
FDCAN_HandleTypeDef *Ingenia_hcan;

#if MOTOR_DRIVER_DEBUG==1
void Ingenia_debug_init(TSerial *port, FDCAN_HandleTypeDef *hcan)
{
	serial = port;
	Ingenia_hcan = hcan;
}
...
HAL_StatusTypeDef Ingenia_begin(FDCAN_HandleTypeDef *hcan)
{
	...
	/* Prepare Tx Header */
//	mtrTxHeader.Identifier = id;
	mtrTxHeader.IdType = FDCAN_STANDARD_ID;
	mtrTxHeader.TxFrameType = FDCAN_DATA_FRAME;
	mtrTxHeader.DataLength = FDCAN_DLC_BYTES_8;
	mtrTxHeader.ErrorStateIndicator = FDCAN_ESI_ACTIVE;
	mtrTxHeader.BitRateSwitch = FDCAN_BRS_OFF;
	mtrTxHeader.FDFormat = FDCAN_CLASSIC_CAN;
	mtrTxHeader.TxEventFifoControl = FDCAN_NO_TX_EVENTS;
	mtrTxHeader.MessageMarker = 0;
...
//	}
//}
HAL_StatusTypeDef Ingenia_IRQHandler(FDCAN_HandleTypeDef *hcan)
{
	CAN_Data_t data;
	uint32_t id;
	uint32_t idType = 0;
	...
	if (HAL_FDCAN_GetRxMessage(hcan, FDCAN_RX_FIFO0, &data.canRxHeader, data.rxData) == HAL_OK) {
		if (data.canRxHeader.IdType == FDCAN_STANDARD_ID) {
			id = data.canRxHeader.Identifier;
			...
			idType = data.canRxHeader.Identifier - id;
...
void Ingenia_write_nmt(Servo_t *servo, NmtModes_e mode)
{
	...
	mtrTxHeader.Identifier = COB_NMT_SERVICE;
	mtrTxHeader.DataLength = FDCAN_DLC_BYTES_2;
	...
	HAL_FDCAN_AddMessageToTxFifoQ(Ingenia_hcan, &mtrTxHeader, canTxBuffer);
}
...
void Ingenia_write_rpdo(Servo_t *servo, uint32_t cob_rpdo, uint8_t *data, uint8_t len)
{
	...
	mtrTxHeader.Identifier = cob_rpdo | servo->_u8Node;
	mtrTxHeader.DataLength = (uint32_t) len << 16;

	memcpy(canTxBuffer, data, len);

	/* Start the Transmission process */
	uint32_t timeout = HAL_GetTick() + 1000;
	while (HAL_FDCAN_AddMessageToTxFifoQ(Ingenia_hcan, &mtrTxHeader, canTxBuffer) != HAL_OK) {
...
uint32_t Ingenia_read_sdo(Servo_t *servo, uint16_t u16Index, uint8_t u8SubIndex, uint8_t *bIsValid)
{
	...
	mtrTxHeader.Identifier = COB_RSDO + servo->_u8Node;
	mtrTxHeader.DataLength = FDCAN_DLC_BYTES_4;
...
#if MOTOR_DRIVER_DEBUG==1
	bufLen = sprintf(buf, "id= 0x%lX: %X %X %X %X\r\n", mtrTxHeader.Identifier, canTxBuffer[0],
			canTxBuffer[1], canTxBuffer[2], canTxBuffer[3]);
	serial_write_str(serial, buf, bufLen);
	...
	while (!isValid) {
		...
		if (millis >= sendTimer) {
			/* Start the Transmission process */
#if MOTOR_DRIVER_DEBUG==0
			HAL_FDCAN_AddMessageToTxFifoQ(Ingenia_hcan, &mtrTxHeader, canTxBuffer);
#else
			if (HAL_FDCAN_AddMessageToTxFifoQ(Ingenia_hcan, &mtrTxHeader, canTxBuffer) == HAL_OK)
				bufLen = sprintf(buf, "send OK!\r\n");
			else
				bufLen = sprintf(buf, "nOK!\r\n");
			serial_write_str(serial, buf, bufLen);
#endif	//if MOTOR_DRIVER_DEBUG==0
...
#if MOTOR_DRIVER_DEBUG==1
		bufLen = sprintf(buf, "send sdo! %d %d\r\n", can_buffer_available(&servo->buffer.bufTSDO),
				isValid);
		serial_write_str(serial, buf, bufLen);
#endif	//if MOTOR_DRIVER_DEBUG==1
	}
...
#if MOTOR_DRIVER_DEBUG==1
	bufLen = sprintf(buf, "status= 0x%X\r\n", u16StatusWord);
	serial_write_str(serial, buf, bufLen);
#endif	//if MOTOR_DRIVER_DEBUG==1
